...
void ksw_extend_batch2_unopt(swrst_t* swrts, uint32_t size, int m, const int8_t *mat, int o_del, int e_del, int o_ins, int e_ins, int zdrop)
{
#ifdef SWDB
    clock_t m_start,m_end;
...
void ksw_extend_batch_origin_unopt(swrst_t* swrts, size_t size, int m, const int8_t *mat, int o_del, int e_del, int o_ins, int e_ins, int zdrop)
{
    //sort
    assert(m==5);
    for(int i=0; i<size;++i)
...
int main(int argc, char** argv)
{
    ...
    rtime = realtime();
    ksw_extend_batch_origin_unopt(nsrt, process_sze, g_m, g_mat[0], g_o_del, g_e_del, g_o_ins, g_e_ins,g_zdrop);
    //time
    fprintf(stderr, "[M::%s]originalunopt Processed %ld reads in %.3f CPU sec, %.3f real sec\n", __func__, nread, cputime() - ctime, realtime() - rtime);
    
    rtime = realtime();
    ksw_extend_batchw2(nsrt, process_sze, g_m, g_mat[0], g_o_del, g_e_del, g_o_ins, g_e_ins,100, 3, g_zdrop);
    //time
    fprintf(stderr, "[M::%s]simdw=100 Processed %ld reads in %.3f CPU sec, %.3f real sec\n", __func__, nread, cputime() - ctime, realtime() - rtime);
    
    rtime = realtime();
    ksw_extend_batchw(nsrt, process_sze, g_m, g_mat[0], g_o_del, g_e_del, g_o_ins, g_e_ins,100, 3, g_zdrop);
    //time
    fprintf(stderr, "[M::%s]originalw=100 Processed %ld reads in %.3f CPU sec, %.3f real sec\n", __func__, nread, cputime() - ctime, realtime() - rtime);
    
    
    rtime = realtime();
    ksw_extend_batchw2(nsrt, process_sze, g_m, g_mat[0], g_o_del, g_e_del, g_o_ins, g_e_ins,200, 3, g_zdrop);
    //time
    fprintf(stderr, "[M::%s]simdw=200 Processed %ld reads in %.3f CPU sec, %.3f real sec\n", __func__, nread, cputime() - ctime, realtime() - rtime);
    
    rtime = realtime();
    ksw_extend_batchw(nsrt, process_sze, g_m, g_mat[0], g_o_del, g_e_del, g_o_ins, g_e_ins,200, 3, g_zdrop);
    //time
    fprintf(stderr, "[M::%s]originalw=200 Processed %ld reads in %.3f CPU sec, %.3f real sec\n", __func__, nread, cputime() - ctime, realtime() - rtime);
